...
class Arcface(object):
    ...
    # 和静态方法差不多, 通过 Arcface.get_defaults 调用
    @classmethod
    ...
    def __init__(self, **kwargs):
        # python 中的__dict__存放了类中所有的属性和其值
    ...
    def detect_image(self, image_1, image_2):
        ...
        with torch.no_grad():
            image_1 = resize_image(image_1, [
                                   self.input_shape[1], self.input_shape[0]], letterbox_image=self.letterbox_image)
            image_2 = resize_image(image_2, [
                                   self.input_shape[1], self.input_shape[0]], letterbox_image=self.letterbox_image)

            photo_1 = torch.from_numpy(np.expand_dims(np.transpose(
                preprocess_input(np.array(image_1, np.float32)), (2, 0, 1)), 0))
            photo_2 = torch.from_numpy(np.expand_dims(np.transpose(
                preprocess_input(np.array(image_2, np.float32)), (2, 0, 1)), 0))
    ...
    def get_FPS(self, image, test_interval):
        ...
        #   归一化+添加上batch_size维度
        # ---------------------------------------------------------#
        image_data = torch.from_numpy(np.expand_dims(np.transpose(
